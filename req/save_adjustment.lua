local save_version = ASS:setting("save_version")

if save_version == ASS:setting("save_version", true) then
	return false
end
...
local funcs = {
	[-1] = function()
		-- random level mod
		local level_mod = ASS:setting("level_mod")
		...
		local dmg_interval = ASS:setting("dmg_interval")
