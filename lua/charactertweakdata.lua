...
function CharacterTweakData:moon_oops_all_bo(is_first_world_problem)
	...
	-- Prevent Bobblehead Bob from running away on First World Bank
	if is_first_world_problem ~= false then
		self.civilian.flee_type = self.bank_manager.flee_type
		self.civilian.run_away_delay = self.bank_manager.run_away_delay
	end
...
if ASS.settings.doms_all_hard then
	...
	-- Swap out easy and normal preset for hard preset
	-- Could replace their data in _presets, but not a fan of that
