			...
			if id:match(prefix) then
				...
				for continent, units in pairs(unit_types) do
					for i, unit in ipairs(units) do
						...
						local replacement_ids = replacement and Idstring(replacement)

						if replacement_ids and units[i] ~= replacement_ids then
							units[i] = replacement_ids
						end
					end
				end
			end
...
function GroupAITweakData:_moon_init_unit_categories_normal()
	self:_moon_swap_units({ CS = "normal", FBI = "normal", })
end
...
function GroupAITweakData:_moon_init_unit_categories_overkill()
	self:_moon_swap_units({ CS = "normal", FBI = "overkill_145", })
...
function GroupAITweakData:_moon_init_unit_categories_overkill_145()
	self:_moon_swap_units({ CS = "overkill_145", FBI = "overkill_145", })
end

function GroupAITweakData:_moon_init_unit_categories_easy_wish()
	self:_moon_swap_units({ CS = "overkill_145", FBI = "overkill_290", })
end

function GroupAITweakData:_moon_init_unit_categories_overkill_290()
	self:_moon_swap_units({ CS = "overkill_290", FBI = "overkill_290", })
...
function GroupAITweakData:_moon_init_unit_categories_sm_wish()
	self:_moon_swap_units({ CS = "sm_wish", FBI = "sm_wish", })
end

function GroupAITweakData:_moon_level_mod_CS_normal()
	self:_moon_swap_units({ CS = "CS_normal", FBI = "CS_normal", })
end

function GroupAITweakData:_moon_level_mod_CS_FBI_overkill()
	self:_moon_swap_units({ CS = "CS_normal", FBI = "FBI_overkill_145", })
end

function GroupAITweakData:_moon_level_mod_FBI_overkill_145()
	self:_moon_swap_units({ CS = "FBI_overkill_145", FBI = "FBI_overkill_145", })
end
...
function GroupAITweakData:_moon_level_mod_FBI_mcmansion()
	self:_moon_swap_units({ CS = "FBI_mcmansion", FBI = "FBI_overkill_145", })
end

function GroupAITweakData:_moon_level_mod_FBI_CITY_easy_wish()
	self:_moon_swap_units({ CS = "FBI_overkill_145", FBI = "CITY_overkill_290", })
end

function GroupAITweakData:_moon_level_mod_CITY_overkill_290()
	self:_moon_swap_units({ CS = "CITY_overkill_290", FBI = "CITY_overkill_290", })
end
...
function GroupAITweakData:_moon_init_unit_categories()
	...
	self.unit_categories.marshal_marksman.special_type = "marshal"
...
function GroupAITweakData:_moon_init_enemy_spawn_groups()
	...
	for id, data in pairs({
		empty = {},
		marshal_marksman = { "shield_cover", "flank", "ranged_fire", "no_push", },
		marshal_shield = { "shield", "flank", "ranged_fire", "no_push", },
...
function GroupAITweakData:_moon_init_task_data()
	...
	if ASS:get_setting("max_balance_muls") then
		local force_balance_mul = self.besiege.assault.force_balance_mul
		local force_pool_balance_mul = self.besiege.assault.force_pool_balance_mul

		local max_force_i = #force_balance_mul
		local max_force_pool_i = #force_pool_balance_mul

		local max_force_mul = force_balance_mul[max_force_i]
		local max_force_pool_mul = force_pool_balance_mul[max_force_pool_i]

		for i = 1, max_force_i do
			force_pool_balance_mul[i] = max_force_mul
		end

		for i = 1, max_force_pool_i do
			force_pool_balance_mul[i] = max_force_pool_mul
		end
	end
...
ASS:post_hook( GroupAITweakData, "init", function(self)
